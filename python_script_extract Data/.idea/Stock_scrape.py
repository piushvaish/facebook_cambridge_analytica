#code to get extract the data from yahoo finance
# -*- coding: utf-8 -*-
"""
...
@author: c0redumb
code from:
https://github.com/c0redumb/yahoo_quote_download/blob/master/README.md
"""
...
#Extract two years of historical stock prices (this can also extract dividends ) and convert the list to the dataframe
df = pd.DataFrame([ x.split(',') for x in load_yahoo_quote("fb","20160403", "20180403", info = 'quote')])
#set the headers
df.columns = df.iloc[0]
#check for duplicates
df[df.duplicated()]  ### No duplicates found
#drop first row and create a dataframe
df= df.drop(df.index[0])
#delete the last empty row
df1= df.drop(df.index[len(df)-1])
df['Year'] = df1['Date'].str.split("-").apply(lambda x: x[0])
df['Month'] = df1['Date'].str.split("-").apply(lambda x: x[1])
df['Day'] = df1['Date'].str.split("-").apply(lambda x: x[2])
